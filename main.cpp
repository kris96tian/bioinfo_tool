...
int main(int argc, char* argv[]) 
{
    ...
    if (argc < 3) {
        std::cout << "Usage: " << argv[0] << " file1 file2 [match] [mismatch] [gap]" << std::endl;
        return 1;
    }
    ...
    // Align sequences and output the results
    for (size_t i = 0; i < std::min(seqs_v.size(), seqs_h.size()); ++i) {
        ...
        std::cout << "Alignment score for sequence " << i + 1 << ": " << score << std::endl;
